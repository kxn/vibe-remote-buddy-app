...
mod win {
    ...
    fn guard(window: &Window) -> Result<HWND, String> {
        let current = crate::desktop::foreground()?;
    ...
    fn live_switch_from_weasel() {
        ...
        let target = crate::desktop::list()
            .unwrap()
            .into_iter()
            .find(|w| w.process.eq_ignore_ascii_case("zcode.exe"))
            .unwrap();
        crate::desktop::activate(&target).unwrap();
        crate::desktop::focus(
            &target,
            &crate::desktop::Focus {
                names: vec![
...
pub use win::switch;
#[cfg(not(windows))]
pub fn switch(_: &crate::desktop::Window, _: &str) -> Result<(), String> {
