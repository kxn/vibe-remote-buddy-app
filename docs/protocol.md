...
语音键（key_id=2）只允许 kind=3/5；其他键不能使用这两种动作，因为蓝牙协议不保证任意按钮能发起录音。
...
- 最多两只持久绑定、两条 BLE 连接；语音仍为一路独占，不承诺同时录音。
- INFO 新增 `max_remotes=2` 和 `capacity_exceeded`。`slots=4` 仅表示可枚举的历史记录 ID 范围，不再表示可添加数量；旧 ID 2/3 仍可读写、解绑、重连，不能按下标截断。
- 升级保留完整绑定、密钥、按键映射和连接缓存。已有三/四只时暂停连接和新增；应用展示全部旧记录，用户删至两只后自动恢复。不会擅自选择保留哪两只，也不移动记录以避免破坏身份和个人设置引用。
- 新增、诊断连接、适配移交、重试、自动连接和最终原子提交共同遵守容量。进行中的连接和诊断连接同样占用无线连接名额。
- 机型知识库容量不变。多接收器聚合属于后续功能。


## 临时语音模式切换（voice_toggle=1）

INFO.voice_toggle=1 表示支持 kind=6，modifiers/value 必须都是 0，仅允许非语音键。
每次实体键按下沿切换本遥控器的语音模式，按住不重复触发；不发送该动作自身的 HID 或上位机动作事件。
普通模式使用保存的语音键配置，会议模式发送无修饰键空格（usage 44）。原配置已是会议模式时，另一模式为豆包预设（kind=5/value=1）。
状态仅在板端 RAM，休眠重连、管理程序启停及相同映射重载保留；断电、删除绑定或更改语音键配置恢复默认。录音已开始时保留该次按下的快捷键，切换影响下一次录音。
MAP_GET、备份和库默认值返回持久配置，不返回临时模式。旧固件保存该动作前要求升级。
