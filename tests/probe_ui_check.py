...
with sync_playwright() as p:
 ...
 for width,height in [(1000,900),(440,760)]:
  ...
  d.get_by_role('button',name='连接并识别',exact=True).click();d.get_by_text('✓ 连接与识别成功',exact=True).wait_for();expect(d.get_by_role('button',name='配置按键',exact=True)).to_be_enabled();assert d.get_by_role('button',name='保存诊断',exact=True).count()==0;d.get_by_role('button',name='配置按键',exact=True).click()
  ...
  for key,usage in [('上',82),('语音',62)]:
   ...
   if key=='语音':
    modal.get_by_role('button',name='下一步：试录').click();modal.get_by_text('请按住语音键说话约 3 秒，然后松开。',exact=True).wait_for()
    ...
    assert modal.get_by_role('button',name='确认',exact=True).is_disabled()
    page.evaluate('window.fixture.failProbeOpcode=0')
    modal.get_by_role('button',name='重新录音',exact=True).click()
    modal.get_by_text('请按住语音键说话约 3 秒，然后松开。',exact=True).wait_for()
    ...
    modal.locator('audio').wait_for();assert modal.get_by_text('请按住语音键说话约 3 秒，然后松开。',exact=True).count()==0;assert modal.get_by_role('button',name='确认',exact=True).is_disabled()
