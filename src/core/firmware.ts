...
export async function validatePackage(
  ...
  if (
    !Number.isInteger(m.data_min) ||
    !Number.isInteger(m.data_max) ||
    info.schema === undefined ||
    info.schema < m.data_min ||
    info.schema > m.data_max
  )
    throw Error("配置版本不兼容");
  if (
    !info.confirmed ||
  ...
  if (
    ...
    throw Error("固件文件不完整");
  const image = Uint8Array.from(pkg.image),
    view = new DataView(image.buffer);
  if (image[0] !== 0xe9 || view.getUint16(12, true) !== 9)
    throw Error("固件不是 ESP32-S3 镜像");
  const text = (start: number, n: number) =>
    new TextDecoder().decode(image.slice(start, start + n)).split("\0")[0];
  if (
...
export async function transferFirmware(
  ...
  try {
    const m = pkg.manifest;
    const begin = await s.command<{ chunk: number }>(UPDATE.BEGIN, {
      target: m.target,
      version: m.version,
      size: m.size,
      sha256: m.sha256,
    ...
    while (true) {
      ...
      progress({ phase: "正在准备", percent: 0, active: true });
      await sleep(100);
    }
    for (let offset = 0; offset < image.length; offset += 192) {
      if (cancelled()) throw Error("更新已取消");
      const chunk = image.slice(offset, offset + 192),
        hex = Array.from(chunk)
          .map((v) => v.toString(16).padStart(2, "0"))
          .join("");
      const reply = await s.command<{ offset: number }>(UPDATE.DATA, {
        offset,
        hex,
      });
      if (reply.offset !== offset + chunk.length)
        throw Error("固件传输位置不一致");
      progress({
        phase: "正在写入",
        percent: Math.floor((reply.offset / image.length) * 95),
        active: true,
      });
    }
    ...
    activated = true;
    try {
      await s.command(UPDATE.ACTIVATE);
    } catch {
